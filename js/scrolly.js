...
(function () {
  ...
  var defaultClickCss =
    " visibility: visible; -webkit-animation-duration: 1s; display:block;-webkit-animation-fill-mode: both; ";
  ...
  this.isInView = function (box) {
    ...
    var attrDataScrollyOffsetTop = box.getAttribute(
      settingOptions.dataScrollyOffsetTop
    );
    ...
    var attrDataScrollyOffsetBottom = box.getAttribute(
      settingOptions.dataScrollyOffsetBottom
    );
          ...
          if (cssProperty == "animation-delay") {
            ...
            if (direction === "topToBottom") {
              currentElm.setAttribute(
                settingOptions.dataScrollyInDelay,
                cssValue
              );
            } else if (direction === "bottomToTop") {
              currentElm.setAttribute(
                settingOptions.dataScrollyOutDelay,
                cssValue
              );
            }
          } else {
            animationCss +=
              " -webkit-" + cssProperty.trim() + ":" + cssValue + "; ";
          }
        }
      }
    }
  ...
  this.isHidden = function (elm) {
    if (
      elm.getAttribute("style") !== null &&
      elm.getAttribute("style").trim() == hiddenCss.trim()
    ) {
      return true;
    }
    return false;
  ...
  this.checkScreenForAnimation = function () {
    if (
      settingOptions.scrollyStopIt.mobile &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.mobile.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.mobile.maxWidth)
    ) {
      settingOptions.scrollyDisable = true;
      this.resetWindow();
    } else if (
      settingOptions.scrollyStopIt.tab &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.tab.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.tab.maxWidth)
    ) {
      settingOptions.scrollyDisable = true;
      this.resetWindow();
    } else if (
      settingOptions.scrollyStopIt.desktop &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.desktop.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.desktop.maxWidth)
    ) {
      settingOptions.scrollyDisable = true;
      this.resetWindow();
    } else if (
      settingOptions.scrollyStopIt.lgDesktop &&
      screenWidth >=
        parseInt(settingOptions.scrollyScreen.lgDesktop.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.lgDesktop.maxWidth)
    ) {
      settingOptions.scrollyDisable = true;
      this.resetWindow();
    } else {
  ...
  this.scrolly = function (options) {
    ...
    if (
      OSName === "Windows" &&
      parseInt(settingOptions.scrollyWindowsOffsetTop)
    ) {
      settingOptions.scrollyOffsetTop = parseInt(
        settingOptions.scrollyWindowsOffsetTop
      );
    }
    //Setting Offset Bottom Value for Windows
    if (
      OSName === "Windows" &&
      parseInt(settingOptions.scrollyWindowsOffsetBottom)
    ) {
      settingOptions.scrollyOffsetBottom = parseInt(
        settingOptions.scrollyWindowsOffsetBottom
      );
    }
    ...
    if (OSName === "Mac" && parseInt(settingOptions.scrollyMacOffsetTop)) {
      settingOptions.scrollyOffsetTop = parseInt(
        settingOptions.scrollyMacOffsetTop
      );
    }
    //Setting Offset Bottom Value for MAC
    if (OSName === "Mac" && parseInt(settingOptions.scrollyMacOffsetBottom)) {
      settingOptions.scrollyOffsetBottom = parseInt(
        settingOptions.scrollyMacOffsetBottom
      );
    }

    //Setting Offset Top Value for Mobile Or Tab
    if (
      parseInt(settingOptions.scrollyMobileOffsetTop) &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.mobile.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.mobile.maxWidth)
    ) {
      settingOptions.scrollyOffsetTop = parseInt(
        settingOptions.scrollyMobileOffsetTop
      );
    } else if (
      parseInt(settingOptions.scrollyTabOffsetTop) &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.tab.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.tab.maxWidth)
    ) {
      settingOptions.scrollyOffsetTop = parseInt(
        settingOptions.scrollyTabOffsetTop
      );
    }

    //Setting Offset Bottom Value for Mobile Or Tab
    if (
      parseInt(settingOptions.scrollyMobileOffsetBottom) &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.mobile.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.mobile.maxWidth)
    ) {
      settingOptions.scrollyOffsetBottom = parseInt(
        settingOptions.scrollyMobileOffsetBottom
      );
    } else if (
      parseInt(settingOptions.scrollyTabOffsetBottom) &&
      screenWidth >= parseInt(settingOptions.scrollyScreen.tab.minWidth) &&
      screenWidth <= parseInt(settingOptions.scrollyScreen.tab.maxWidth)
    ) {
      settingOptions.scrollyOffsetBottom = parseInt(
        settingOptions.scrollyTabOffsetBottom
      );
    }

    // Get & Set All Css Animation Click Event Funcition
    var dataInElm = document.querySelectorAll(
      "[" + settingOptions.dataSelectorIn + "]"
    );
    var dataInElmCount = dataInElm.length;

    // Get & Set All Css Animation Click Event Funcition
    var dataOutElm = document.querySelectorAll(
      "[" + settingOptions.dataSelectorOut + "]"
    );
    var dataOutElmCount = dataOutElm.length;

    // Get & Set All Scrolly Animation Click Event with Target Funcition
    var dataScrollyTargetClickElm = document.querySelectorAll(
      "[" + settingOptions.dataScrollyTargetClick + "]"
    );
    var dataScrollyTargetElmCount = dataScrollyTargetClickElm.length;

    if (dataScrollyTargetElmCount) {
      for (
        var dataScrollyTargetElmIndex = 0;
        dataScrollyTargetElmIndex < dataScrollyTargetElmCount;
        dataScrollyTargetElmIndex++
      ) {
        dataScrollyTargetClickElm[dataScrollyTargetElmIndex].addEventListener(
          "click",
        ...
        if (
          !this.hasClass(
            dataScrollyTargetClickElm[dataScrollyTargetElmIndex],
            settingOptions.selectorClass
          )
        ) {
          dataScrollyTargetClickElm[dataScrollyTargetElmIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }

    var dataScrollyTargetMouseOverElm = document.querySelectorAll(
      "[" + settingOptions.dataScrollyTargetMouseOver + "]"
    );
    var dataScrollyTargetMouseOverElmCount =
      dataScrollyTargetMouseOverElm.length;

    if (dataScrollyTargetMouseOverElmCount) {
      for (
        var dataScrollyTargetMouseOverElmIndex = 0;
        dataScrollyTargetMouseOverElmIndex < dataScrollyTargetMouseOverElmCount;
        dataScrollyTargetMouseOverElmIndex++
      ) {
        dataScrollyTargetMouseOverElm[
          dataScrollyTargetMouseOverElmIndex
        ].addEventListener("mouseover", scrollyAnimationTarget, true);

        if (
          !this.hasClass(
            dataScrollyTargetMouseOverElm[dataScrollyTargetMouseOverElmIndex],
            settingOptions.selectorClass
          )
        ) {
          dataScrollyTargetMouseOverElm[
            dataScrollyTargetMouseOverElmIndex
          ].className += " " + settingOptions.selectorClass;
        }
      }
    }

    var dataScrollyTargetMouseOutElm = document.querySelectorAll(
      "[" + settingOptions.dataScrollyTargetMouseOut + "]"
    );
    ...
    if (dataScrollyTargetMouseOutElmCount) {
      for (
        var dataScrollyTargetElmIndex = 0;
        dataScrollyTargetElmIndex < dataScrollyTargetMouseOutElmCount;
        dataScrollyTargetElmIndex++
      ) {
        dataScrollyTargetMouseOutElm[
          dataScrollyTargetElmIndex
        ].addEventListener("mouseout", scrollyAnimationTarget, true);
        if (
          !this.hasClass(
            dataScrollyTargetMouseOutElm[dataScrollyTargetElmIndex],
            settingOptions.selectorClass
          )
        ) {
          dataScrollyTargetMouseOutElm[dataScrollyTargetElmIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }
    ...
    // Get & Set All Css Animation Click Event Funcition
    var clickElm = document.querySelectorAll(
      "[" + settingOptions.dataClickElm + "]"
    );
    ...
    if (clickElmCount) {
      for (
        var clickElmIndex = 0;
        clickElmIndex < clickElmCount;
        clickElmIndex++
      ) {
        clickElm[clickElmIndex].addEventListener(
          "click",
          this.cssAnimationClick
        );
        if (
          !this.hasClass(clickElm[clickElmIndex], settingOptions.selectorClass)
        ) {
          clickElm[clickElmIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }
    ...
    // Get & Set All Css Animation Click Event Funcition
    var mouseOverElm = document.querySelectorAll(
      "[" + settingOptions.dataMouseoverElm + "]"
    );
    var mouseOverElmCount = mouseOverElm.length;

    if (mouseOverElmCount) {
      for (
        var mouseOverElmIndex = 0;
        mouseOverElmIndex < mouseOverElmCount;
        mouseOverElmIndex++
      ) {
        mouseOverElm[mouseOverElmIndex].addEventListener(
          "mouseover",
          this.cssAnimationMouseOver
        );
        if (
          !this.hasClass(
            mouseOverElm[mouseOverElmIndex],
            settingOptions.selectorClass
          )
        ) {
          mouseOverElm[mouseOverElmIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }
    ...
    // Get & Set All Css Animation Click Event Funcition
    var mouseOutElm = document.querySelectorAll(
      "[" + settingOptions.dataMouseoutElm + "]"
    );
    var mouseOutElmCount = mouseOutElm.length;
    if (mouseOutElmCount) {
      for (
        var mouseOutElmCountIndex = 0;
        mouseOutElmCountIndex < mouseOutElmCount;
        mouseOutElmCountIndex++
      ) {
        mouseOutElm[mouseOutElmCountIndex].addEventListener(
          "mouseout",
          this.cssAnimationMouseOut
        );

        if (
          !this.hasClass(
            mouseOutElm[mouseOutElmCountIndex],
            settingOptions.selectorClass
          )
        ) {
          mouseOutElm[mouseOutElmCountIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }
    ...
    if (dataInElmCount) {
      for (
        var dataInElmCountIndex = 0;
        dataInElmCountIndex < dataInElmCount;
        dataInElmCountIndex++
      ) {
        if (
          !this.hasClass(
            dataInElm[dataInElmCountIndex],
            settingOptions.selectorClass
          )
        ) {
          dataInElm[dataInElmCountIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }
    ...
    if (dataOutElmCount) {
      for (
        var dataOutElmCountIndex = 0;
        dataOutElmCountIndex < dataOutElmCount;
        dataOutElmCountIndex++
      ) {
        if (
          !this.hasClass(
            dataOutElm[dataOutElmCountIndex],
            settingOptions.selectorClass
          )
        ) {
          dataOutElm[dataOutElmCountIndex].className +=
            " " + settingOptions.selectorClass;
        }
      }
    }
    ...
    if (sCount) {
      for (var i = 0; i < sCount; i++) {
        ...
        var attrDataCharacterAnimate = selector[i].getAttribute(
          settingOptions.dataCharacterAnimate
        );
        ...
        if (attrDataCharacterAnimate) {
          ...
          var charecterAttrInData = selector[i].getAttribute(
            settingOptions.dataSelectorIn
          );
          var charecterAttrOutData = selector[i].getAttribute(
            settingOptions.dataSelectorOut
          );
          ...
          for (var j = 0; j < innerText.length; j++) {
            ...
            if (str != " ") {
              ...
              var newIndex = i + "-ca-" + j;
              var styleCss =
                " display: inline-block; animation-fill-mode: both; -webkit-animation-duration:1s; -webkit-animation-delay:" +
                ad +
                "ms; ";
          ...
          // make animation in css
          var attrInData = selector[i].getAttribute(
            settingOptions.dataSelectorIn
          );
          if (attrInData) {
          ...
          var attrOutData = selector[i].getAttribute(
            settingOptions.dataSelectorOut
          );
          if (attrOutData) {
            animationOutStyleMap[i] = this.makeStyle(
              attrOutData,
              "bottomToTop"
            );
  ...
  this.showElement = function (elmIndex) {
    if (this.isHidden(selector[elmIndex])) {
      var isCharacterAnimate = selector[elmIndex].getAttribute(
        settingOptions.dataCharacterAnimate
      );
      if (topToBottom) {
        // delay element show
        if (
          selector[elmIndex].getAttribute(settingOptions.dataScrollyOutDelay)
        ) {
          var timeInTime =
            parseFloat(
              selector[elmIndex].getAttribute(
                settingOptions.dataScrollyOutDelay
              )
            ) * 1000;
          ...
          dataScrollyInDelayTimer = setTimeout(function () {
            if (isCharacterAnimate) {
              selector[elmIndex].style.cssText =
                displayCss + tagStyleMap[elmIndex] + characterCss;
              if (
                !this.hasClass(
                  selector[elmIndex],
                  characterAnimateInClassMap[elmIndex]
                )
              ) {
                selector[elmIndex].className +=
                  characterAnimateInClassMap[elmIndex];
              }
            } else {
              selector[elmIndex].style.cssText =
                animationOutStyleMap[elmIndex] + tagStyleMap[elmIndex];
            }
          ...
          if (isCharacterAnimate) {
            ...
            selector[elmIndex].style.cssText =
              displayCss +
              animationOutStyleMap[elmIndex] +
              tagStyleMap[elmIndex];
          }
          selector[elmIndex].style.visibility = "visible";
        }
      } else {
        // delay element show
        if (
          selector[elmIndex].getAttribute(settingOptions.dataScrollyInDelay)
        ) {
          var timeOutTime =
            parseFloat(
              selector[elmIndex].getAttribute(settingOptions.dataScrollyInDelay)
          ...
          dataScrollyOutDelayTimer = setTimeout(function () {
            if (isCharacterAnimate) {
              ...
              if (
                !this.hasClass(
                  selector[elmIndex],
                  characterAnimateOutClassMap[elmIndex]
                )
              ) {
                selector[elmIndex].className +=
                  characterAnimateOutClassMap[elmIndex];
              ...
              selector[elmIndex].style.cssText =
                animationInStyleMap[elmIndex] + tagStyleMap[elmIndex];
            }
            selector[elmIndex].style.visibility = "visible";
          }, timeOutTime);
        } else {
  ...
  var scrollyAnimationTarget = function () {
    ...
    var clickTagTarget = clickTag.getAttribute(
      settingOptions.dataScrollyTargetClick
    );

    if (clickTagTarget == null) {
      clickTagTarget = clickTag.getAttribute(
        settingOptions.dataScrollyTargetMouseOver
      );
    }
    ...
    var dataScrollyTargetElm = document.querySelector(
      "[" + settingOptions.dataScrollySelector + "=" + clickTagTarget + "]"
    );

    var thisElm = dataScrollyTargetElm;
    var clickEvnCssName = clickTag.getAttribute(
      settingOptions.dataScrollyAnimation
    );
    ...
    if (thisElm.getAttribute(settingOptions.dataScrollyIndex) === null) {
      ...
      thisElm.setAttribute(
        settingOptions.dataScrollyIndex,
        tagStyleMapSizeIndex
      );
  ...
  this.cssAnimtionEnd = function () {
    var thisTag = this;
    var thisIndex = parseInt(
      thisTag.getAttribute(settingOptions.dataScrollyIndex)
    );
  ...
  this.cssAnimationClick = function () {
    var clickTag = this;
    var clickEvnCssName = clickTag
      .getAttribute(settingOptions.dataClickElm)
      .trim();
    clickTag.removeAttribute("style");
    setTimeout(function () {
      clickTag.style.cssText =
        defaultClickCss + " animation-name: " + clickEvnCssName + ";";
    }, 0);
  };
  ...
  this.cssAnimationMouseOver = function () {
    var clickTag = this;
    var clickEvnCssName = clickTag
      .getAttribute(settingOptions.dataMouseoverElm)
      .trim();
    clickTag.removeAttribute("style");
    setTimeout(function () {
      clickTag.style.cssText =
  ...
  this.cssAnimationMouseOut = function () {
    var clickTag = this;
    var clickEvnCssName = clickTag
      .getAttribute(settingOptions.dataMouseoutElm)
      .trim();
    clickTag.removeAttribute("style");
    setTimeout(function () {
      clickTag.style.cssText =
